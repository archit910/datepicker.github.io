  ...
  return {
    ...
    link: function (scope, element, attribute) {
      ...
      var basicConfiguration = function () {
        ...
        scope.timepicker === true ? ng.noop() :  false;
      ...
      scope.$watch('mode', function () {
        if (scope.mode === 'single' && !(scope.timerange instanceof Date)) {
          scope.timerange = new Date(scope.timerange.date);
        } else if (scope.mode === 'range' && (scope.timerange instanceof Date)) {
      ...
      scope.$watch('startofweek', function () {
        basicConfiguration();
      });
    },
    controller: function ($scope) {
      var timeValid = function (timeModel, clk) {
        var flag = true;
        if (clk === 12) {
          if (timeModel.hours > 12 || timeModel.hours < 0) {
            flag = false;
          }
        } else if (clk === 24) {
          if (timeModel.hours > 24 || timeModel.hours < 0) {
            flag = false;
          }
        }
        if (timeModel.minutes > 60 || timeModel.minutes < 0) {
          flag = false;
        }
        if (timeModel.seconds > 60 || timeModel.seconds < 0) {
          flag = false;
        }
        return flag;
      };
...
function parsetime() {
  return {
    ...
    link: function (scope, ele, attr, ngModel) {
      ...
      if(attr.ngModel.split('.')[1] === 'hours') {
        if (val > 24) {
          ngModel.$setViewValue('01');
          ngModel.$render();
          return;
        }
      } else {
      ...
      ngModel.$setViewValue(('0' + val).slice(-2));
      ngModel.$render();
